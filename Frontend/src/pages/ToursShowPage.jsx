...
import { Search, MapPin, Calendar, Users, Star, Filter, Heart, X, Sparkles } from 'lucide-react';
...
const ToursShowPage = () => {
  ...
  const defaultFilters = {
    price: [0, 50000000], // 0 - 50 triệu
    duration: [0, 14],     // 0 - 14 ngày
    groupSize: 1,
    rating: 0,           // 0 = All
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/locations/`)
      .then(res => res.json())
      .then(data => setLocations(data.map(p => ({ id: p.id, name: p.name }))))
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/tours/?${params.toString()}`)
      .then(res => res.json())
      .then(data => {
        setTours(data);
        setIsLoading(false);
        ...
        <div className="flex flex-wrap items-center gap-4 mb-8">
          ...
          <Dialog open={isFilterDialogOpen} onOpenChange={setIsFilterDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Bộ lọc chi tiết</DialogTitle>
              </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 py-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>{formatPrice(tempFilters.price[0])}</span>
                    <span>{formatPrice(tempFilters.price[1])}</span>
                  </div>
                </div>

                {/* 2. Duration */}
                <div className="space-y-3">
                  <Label>Thời lượng (số ngày)</Label>
                  <Slider
                  ...
                  />
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>{tempFilters.duration[0]} ngày</span>
                    <span>{tempFilters.duration[1]} ngày</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value) => setTempFilters(prev => ({ ...prev, rating: Number(value) }))}
                  >
                    {[4, 3, 2, 1, 0].map(star => (
                      <div key={star} className="flex items-center space-x-2">
                        <RadioGroupItem value={String(star)} id={`r-${star}`} />
                        <Label htmlFor={`r-${star}`} className="font-normal">
                          {star > 0 ? `Từ ${star} sao trở lên` : "Tất cả"}
                        </Label>
                      </div>
                    ))}
                  </RadioGroup>
                ...
                <div className="space-y-3">
                  <Label>Số người trong đoàn</Label>
                   <Input
                      ...
                      onChange={(e) => setTempFilters(prev => ({...prev, groupSize: Number(e.target.value) || 1}))}
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2">
                    {filterOptions.transportation.map(opt => (
                      <div key={opt.value} className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor={`trans-${opt.value}`} className="font-normal">{opt.label}</Label>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <DialogFooter>
                <Button variant="ghost" onClick={handleClearFilters}>Xóa tất cả</Button>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center mb-4">
                  ...
                  <span className="text-sm text-gray-600 ml-2">
                    {tour.rating.toFixed(1)} ({tour.reviews} đánh giá)
                  </span>
                </div>
