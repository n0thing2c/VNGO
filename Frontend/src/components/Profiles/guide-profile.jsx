...
const FormField = ({ label, children }) => (
  <div className="flex flex-col space-y-1.5">
    <label htmlFor={children.props.id} className="text-sm font-medium text-gray-700">
      {label}
    </label>
    {children}
  </div>
...
export function GuideProfile({ className }) {
  ...
  const formElementClasses = "flex w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50 transition duration-150 ease-in-out shadow-sm";
        ...
        <CardContent className="p-0">
          <form className="p-4 md:p-8 space-y-6" onSubmit={handleSave}>
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              <h2 className="text-lg font-semibold text-gray-800">Add/change profile picture</h2>
              <div className="relative h-28 w-28 rounded-full overflow-hidden border-4 border-white ring-2 ring-gray-300 shadow-md">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = "https://placehold.co/112x112/A0A0A0/ffffff?text=User";
                  }}
                />
              </div>
              ...
              >
                {isSaving ? (
                  <div className="flex items-center">
                    <svg className="animate-spin h-4 w-4 mr-2 border-b-2 border-white rounded-full" viewBox="0 0 24 24"></svg>
                    Saving...
                  </div>
                ) : (
                  "Save"
